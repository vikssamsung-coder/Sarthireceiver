#!/usr/bin/env python3
"""
Bigul · Sarthi CDP · Standalone Two-Month Client Profiling Report
============================================
...
DEFAULT_LEADS_CANDIDATES = [
    Path(r"D:\Sarthi\Leads\Leads.csv"),
    Path(r"C:\Users\Vikrant.Dale\Downloads\Leads.csv"),
    Path(r"D:\Sarthi\Leads.csv"),
]
...
def resolve_path(
    ...
    if required:
        raise FileNotFoundError(
            "Required Leads.csv was not found. Pass its path with --leads."
        )
    return None
...
def get_db_config() -> dict:
    try:
        ...
        if not config["password"]:
            # Reuse the local fallback already configured for Client 360. Read
            # only the constant assignment; never import or execute that file.
            extractor = Path(__file__).with_name("sarthi_new_clients_360_extract.py")
            try:
                ...
                for node in tree.body:
                    ...
                    if isinstance(node.value, ast.Constant) and node.value.value:
                        config["password"] = str(node.value.value)
                    break
...
def main() -> int:
    ...
    print("=" * 88)
    print("BIGUL · SARTHI CDP · NEW CLIENT PROFILING REPORT")
    print("=" * 88)
